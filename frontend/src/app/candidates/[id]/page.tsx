...
export default function CandidateProfilePage({
  ...
  if (isLoading) {
    return (
      <PageScaffold
        ...
        subtitle="Loading profile from backend..."
      >
  ...
  if (isError || !data) {
    return (
      <PageScaffold
        title="Candidate Profile"
        subtitle="Unable to load candidate profile."
      >
        <section className="glass-card rounded-3xl p-5 text-sm text-rose-300">
          Could not load candidate from backend. Please check API and candidate
          ID.
        </section>
      </PageScaffold>
    );
  ...
  return (
    <PageScaffold
      title="Candidate Profile"
      subtitle="Deep AI profile with resume intelligence and communication timeline."
    >
      <section className="grid gap-4 xl:grid-cols-12">
        <article className="glass-card rounded-3xl p-5 xl:col-span-8">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-slate-100">
              {String(candidate.name ?? candidate.email)}
            </h2>
          ...
          <p className="mt-2 text-sm text-slate-400">
            {String(candidate.description ??
              candidate.summary ??
              "Candidate summary will appear once AI processing is complete.")}
          </p>
          <div className="mt-4 rounded-2xl border border-slate-700/60 bg-slate-900/55 p-4 text-sm text-slate-300">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p>{String(candidate.role ?? "Unknown")}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500 uppercase">Experience</p>
                <p>{String(candidate.experience ?? "Unknown")}</p>
              </div>
        ...
        <article className="glass-card rounded-3xl p-5 xl:col-span-4">
          ...
          <ul className="mt-3 space-y-4 text-sm text-slate-300">
            <li>
              ...
              <p className="font-medium">{String(candidate.status ?? "NEW")}</p>
            </li>
            <li>
              <p className="text-xs text-slate-500 uppercase">AI Score</p>
              <p className="font-medium">{String(candidate.score ?? 0)}/100</p>
            </li>
            <li>
              <p className="text-xs text-slate-500 uppercase">Notice Period</p>
              <p className="font-medium">{String(candidate.notice_period ?? "Pending")}</p>
            </li>
            <li>
              <p className="text-xs text-slate-500 uppercase">
                Expected Salary
              </p>
              <p className="font-medium">{String(candidate.expected_salary ?? "Pending")}</p>
            </li>
          </ul>
        </article>
      </section>
      <section className="mt-4 grid gap-4 md:grid-cols-2">
        <article className="glass-card rounded-3xl p-5">
          ...
          <p className="mt-2 text-sm text-slate-400">
            {String(candidate.interview_notes ?? "No interview notes captured yet.")}
          </p>
        </article>
        <article className="glass-card rounded-3xl p-5">
          <h3 className="font-medium text-slate-100">Communication Timeline</h3>
          <p className="mt-2 text-sm text-slate-400">
            Created: {String(candidate.created_at ?? "N/A")} | Updated:{" "}
            {String(candidate.updated_at ?? "N/A")}
          </p>
        </article>
      </section>
    </PageScaffold>
